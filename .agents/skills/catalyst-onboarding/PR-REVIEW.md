...
## What CI catches (you don't need to check)

| job | what it runs | what it catches |
|---|---|---|
| build | `build:tokens` → `build:core` | Type errors, broken imports, SCSS compile failures |
| lint | prettier, ESLint, stylelint | Formatting drift, ESLint violations, SCSS violations |
| test | spec, screenshot, e2e | HTML regression, visual regression, interaction regression |

**Note:** commitlint runs as a Husky pre-commit hook, not in CI. A contributor
who used `--no-verify` bypasses it. Always check commit messages on PRs from
...
**Boilerplate — every interactive component**
- [ ] Has `testId?: string` prop — applied as `data-test` on the native inner element
- [ ] Has `nativeAttributes?: { [key: string]: string }` prop — spread on the native inner element
