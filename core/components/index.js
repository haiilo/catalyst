export { g as getAssetPath, s as setAssetPath, a as setNonce, b as setPlatformOptions } from './p-DJz_AlH8.js';
import { c as catI18nRegistry } from './p-DYxciDq0.js';
export { C as CatI18nRegistry } from './p-DYxciDq0.js';
export { C as CatIconRegistry, c as catIconRegistry } from './p-Cj7g8Tvk.js';
...
class CatNotificationService {
    ...
    show(content, options) {
        ...
        ref.toast.showToast();
        return (result) => ref.toast?.hideToast(result);
    }
    getNode(content, ref, options) {
      ...
      ${options?.autoClose === false
            ? `<cat-button class="cat-toastify-close cat-button-pull" size="s" icon="$cat:notification-close" variant="text" icon-only="true" class="close" a11y-label="${catI18nRegistry.t('notification.dismiss')}"></cat-button>`
            : ''}
    </div>`;
    ...
    getOptions(node, ref, options) {
        const [gravity, position] = (options?.placement?.split('-') ?? ['bottom', 'left']);
...
const catNotificationService = CatNotificationService.getInstance();
 ...
 */
function stringArrayConnector(data) {
 ...
 */
function stringMapConnector(data) {
