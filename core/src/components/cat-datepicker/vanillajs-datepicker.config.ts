...
function daysForLocale(weekday: 'long' | 'short' | 'narrow' = 'long') {
  ...
  return [...Array(7).keys()].map((day: number) => {
    firstDayOfWeek.setUTCDate(firstDayOfWeek.getUTCDate() + day);
...
function monthsForLocale(month: 'long' | 'short' = 'long') {
  ...
  return [...Array(12).keys()].map((month: number) => format(date.setUTCMonth(month)));
}
